...
{
    if (!dialogueEnded && (Engine::GetInstance().input->GetKey(SDL_SCANCODE_SPACE) == KEY_DOWN || Engine::GetInstance().input.get()->GetControllerButton(SDL_CONTROLLER_BUTTON_A) == KEY_DOWN)&& !isBranching)
    {
