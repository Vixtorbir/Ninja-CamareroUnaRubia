...
bool Enemy::Update(float dt)
{
    ...
    // Dibuja la l�nea de visi�n del enemigo
    DrawLineOfSight();
...
bool Enemy::IsNextTileCollidable() {
	...
	// Calcular la posici�n de la siguiente tile en la direcci�n actual
...
bool Enemy::IsPlayerInRange() {
	...
	// Verificar si el enemigo est� mirando en la direcci�n del jugador
...
void Enemy::PerformAttack() {
	...
	if (attackHitbox == nullptr) {
		// Determina la direcci�n del ataque
		Vector2D playerPos = Engine::GetInstance().scene.get()->player->GetPosition();
		Vector2D enemyPos = GetPosition();
		int attackOffsetX = 0;
		...
		if (playerPos.getX() < enemyPos.getX()) {
			...
			attackOffsetX = -80; // Ajusta este valor seg�n sea necesario
		}
		else {
			// El jugador est� a la derecha del enemigo
			attackOffsetX = 110; // Ajusta este valor seg�n sea necesario
		}
	...
	// Agrega la posici�n actual al trazo de la espada
...
bool Enemy::IsPlayerInLineOfSight() {
	...
	// Verifica si el jugador est� dentro de la l�nea de visi�n de 5 tiles
	if (distance <= 10) {
		if (direction == 0 && playerTilePos.getX() < enemyTilePos.getX()) { // Izquierda
			for (int i = 1; i <= 5; ++i) {
				if (Engine::GetInstance().map.get()->IsTileCollidable(enemyTilePos.getX() - i, enemyTilePos.getY())) {
					return false; // Hay una colisi�n en la l�nea de visi�n
				}
				if (playerTilePos.getX() == enemyTilePos.getX() - i) {
					return true; // El jugador est� en la l�nea de visi�n
				}
			}
		}
...
void Enemy::DrawLineOfSight() {
	...
	SDL_SetRenderDrawColor(renderer, 255, 0, 0, 255); // Color rojo para la l�nea de visi�n
	...
	for (int i = 1; i <= 5; ++i) {
		...
		if (Engine::GetInstance().map.get()->IsTileCollidable(tileX, tileY)) {
			break; // Det�n el dibujo si hay una colisi�n en la l�nea de visi�n
		}
